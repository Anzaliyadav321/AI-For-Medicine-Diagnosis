    ...
    for c in range(image.shape[0]):
        ...
        for z in range(image.shape[3]):
            ...
            if np.std(centered) != 0:
                ...
                # if standard deviation is zero, just use the centered image
...
def dice_coefficient(y_true, y_pred, axis=(1, 2, 3), 
    ...
    dice_numerator =  2 * K.sum(y_true * y_pred, axis=axis)
    dice_denominator = K.sum(y_true, axis=axis) + K.sum(y_pred, axis=axis)
    dice_per_class = (dice_numerator + epsilon) / (dice_denominator + epsilon)
    dice_coefficient = K.mean(dice_per_class)
...
def soft_dice_loss(y_true, y_pred, axis=(1, 2, 3), 
    ...
    dice_loss = 1 - K.mean(dice_coefficient)
